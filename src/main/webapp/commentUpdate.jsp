...
commentBean getcomment = comment.getcomment(comment_number);
												...
												<div class="row">
													<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
														<tbody>
																...
																<td align="right">  
																	...
																	%> 
																		 <button class="btn btn-outline-primary" type="button"
																				  onclick="location.href='commentUpdate.jsp?comment_number=<%=list.getComment_number()%>' ">댓글수정</button>
																			<button class="btn btn-outline-primary" type="button"
															...
															<tr>
															<form method="post" action="commentUpdatePro.jsp">
															<td align="left"> <input type="text" style="height:100px;" class="form-control" placeholder="댓글 작성" name ="comment_content" value='<%=getcomment.getComment_content()%>'></td>
															 <input type="hidden" name="number" value='<%=getcomment.getNumber()%>' />
															 <input type="hidden" name="user_id" value='<%=getcomment.getUser_id()%>' /> 
						  									 <input type = "hidden" name = "date" value='<%=getcomment.getComment_date()%>' />
						  									  <input type = "hidden" name = "comment_number" value='<%=getcomment.getComment_number()%>' />
															<td>    <div class="submit"> 
           				    									  <input type="submit" value="수정하기"> </div>  </td>
															</form>
															</tr>
															<%
															} 
